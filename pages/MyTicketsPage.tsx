...
const TicketCard: React.FC<{ ticket: Ticket }> = ({ ticket }) => (
    <div className="bg-white p-5 rounded-2xl shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="text-right w-full sm:w-auto flex flex-row-reverse sm:flex-col items-center sm:items-end justify-between">
            <span className={`text-xs font-semibold px-2 py-1 rounded-full ${ticket.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{ticket.status}</span>
            <button className="mt-0 sm:mt-2 px-4 py-2 bg-orange-500 text-white font-semibold rounded-full text-sm hover:bg-orange-600 transition-colors">
                {ticket.status === 'ACTIVE' ? 'View Details' : 'View Receipt'}
            </button>
        </div>
    </div>
);
...
export const MyTicketsPage: React.FC = () => {
    ...
    const upcomingTickets = USER_TICKETS.filter(t => t.status === 'ACTIVE');
    const pastTickets = USER_TICKETS.filter(t => t.status === 'COMPLETED');
                ...
                <div className="lg:col-span-2">
                    <div className="flex border-b border-gray-200 mb-6">
                     ...
                     <div className="space-y-6">
                        {(activeTab === 'upcoming' ? upcomingTickets : pastTickets).map(ticket => <TicketCard key={ticket.id} ticket={ticket} />)}
                    </div>
                </div>
                <div className="space-y-6">
